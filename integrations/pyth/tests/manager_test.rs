...
async fn test_manager_removing_an_asset() {
    ...
    tokio::spawn(async move {
        ...
        let headers = "HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nConnection: keep-alive\r\n\r\n";
        socket1.write_all(headers.as_bytes()).await.unwrap();
        socket1.write_all(sse_event(known_feeds::AAPL_USD, "220").as_bytes()).await.unwrap();
        socket1.flush().await.unwrap();
        ...
        assert!(!req.contains(known_feeds::TSLA_USD), "TSLA must be excluded after removal");

        socket2.write_all(headers.as_bytes()).await.unwrap();
        socket2.write_all(sse_event(known_feeds::AAPL_USD, "225").as_bytes()).await.unwrap();
        socket2.flush().await.unwrap();
    });
...
async fn test_manager_replacing_a_feed() {
    ...
    tokio::spawn(async move {
        ...
        let headers = "HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nConnection: keep-alive\r\n\r\n";
        socket1.write_all(headers.as_bytes()).await.unwrap();
        socket1.write_all(sse_event(old_feed, "100").as_bytes()).await.unwrap();
        ...
        socket2.write_all(headers.as_bytes()).await.unwrap();
        socket2.write_all(sse_event(new_feed, "200").as_bytes()).await.unwrap();
    });
...
async fn test_manager_empty_subscription_idling() {
    ...
    tokio::spawn(async move {
        ...
        let headers = "HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nConnection: keep-alive\r\n\r\n";
        socket.write_all(headers.as_bytes()).await.unwrap();
        socket.write_all(sse_event(known_feeds::AAPL_USD, "225").as_bytes()).await.unwrap();
    });
    ...
    assert_eq!(server_connected.load(Ordering::SeqCst), 0, "No connection when subscription is empty");

    // Add subscription
    let set = SubscriptionSet::new(vec![make_sub("backed:AAPLx", "AAPL", known_feeds::AAPL_USD)]);
...
async fn test_manager_pyth_disconnect_and_reconnect() {
    ...
    tokio::spawn(async move {
        ...
        let headers = "HTTP/1.1 200 OK\r\nContent-Type: text/event-stream\r\nConnection: close\r\n\r\n";
        socket1.write_all(headers.as_bytes()).await.unwrap();
        socket1.write_all(sse_event(known_feeds::AAPL_USD, "100").as_bytes()).await.unwrap();
        socket1.flush().await.unwrap();
        ...
        let (mut socket2, _) = listener.accept().await.unwrap();
        socket2.write_all(headers.as_bytes()).await.unwrap();
        socket2.write_all(sse_event(known_feeds::AAPL_USD, "105").as_bytes()).await.unwrap();
        socket2.flush().await.unwrap();
    });

    // Short backoff for fast testing
    let config = StreamManagerConfig::new(format!("http://{}", addr))
        .with_backoff(Duration::from_millis(20), Duration::from_millis(100), 2.0);
