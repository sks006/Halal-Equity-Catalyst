...
use equity_catalyst_pyth::{
    known_feeds, DynamicStreamManager, StreamManagerConfig,
};
...
async fn test_phase_5_end_to_end_acceptance_criteria() {
    ...
    tokio::spawn(async move {
        ...
        assert!(!req1.contains(NVDA_FEED), "Session 1 must NOT subscribe to NVDA yet");

        socket1.write_all(headers.as_bytes()).await.unwrap();
        socket1.write_all(sse_event(AAPL_FEED, "22500000000").as_bytes()).await.unwrap();
        ...
        assert!(req2.contains(NVDA_FEED), "Session 2 must subscribe to newly added NVDA");

        socket2.write_all(headers.as_bytes()).await.unwrap();
        socket2.write_all(sse_event(NVDA_FEED, "12500000000").as_bytes()).await.unwrap();
    ...
    let manager_config = StreamManagerConfig::new(hermes_url)
        .with_backoff(Duration::from_millis(20), Duration::from_millis(100), 2.0);

    let manager = DynamicStreamManager::with_channel(
        manager_config,
        watcher.subscribe(),
        price_tx,
    );
    ...
    let nvda_req = sample_asset_request("backed:NVDAx", "NVDA", "MintNVDA", AssetApprovalStatus::ShariahApproved);
