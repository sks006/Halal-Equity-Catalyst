...
fn setup_test_context() -> (axum::Router, Pool, Option<redis::Client>, Option<SolanaService>) {
...
async fn test_health_monitor_service_probes() {
    ...
    assert_eq!(report.components.len(), 8, "Must probe all 8 required components");
    ...
    let pg_comp = report.components.iter().find(|c| c.name == "PostgreSQL").unwrap();
    assert_eq!(pg_comp.status, HealthStatus::Healthy);
    assert!(pg_comp.latency_ms.is_some());

    // Workers must report active/ready state
    let el_comp = report.components.iter().find(|c| c.name == "event listener").unwrap();
    assert_eq!(el_comp.status, HealthStatus::Healthy);

    let pw_comp = report.components.iter().find(|c| c.name == "policy worker").unwrap();
    assert_eq!(pw_comp.status, HealthStatus::Healthy);

    let ew_comp = report.components.iter().find(|c| c.name == "execution worker").unwrap();
