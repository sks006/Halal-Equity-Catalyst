...
export default function AgentPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="p-5 rounded-xl bg-white border border-slate-200 text-slate-900 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-semibold text-slate-900">
              Governance & Security Protection
            </div>
            <p className="text-xs text-slate-500 mt-0.5">
              The AI model never accesses private keys, modifies risk limits, or self-authorizes transactions. Every proposal is subjected to a deterministic validation gate before simulation.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">{decisions.length} Decisions Logged</span>
          </div>
              ...
              decisions.map((dec) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded border ${actionColor}`}>
                          {dec.action}
                        </span>
                        <span className="font-semibold text-xs text-slate-900">{dec.symbol}</span>
                      </div>
                      <span className="text-xs text-slate-400">{dec.timestamp}</span>
                    </div>
              ...
              <CardHeader className="p-6 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-slate-400 font-mono">{selectedDecision.id}</div>
                  </div>
                ...
                <div>
                  ...
                  <div className="p-4 rounded-xl bg-indigo-50/50 border border-indigo-100 text-xs text-slate-800 leading-relaxed space-y-3">
                    ...
                    <div className="border-t border-indigo-100/60 pt-2 space-y-1.5">
                      <span className="text-xs font-semibold text-indigo-900">Market Signals Analyzed:</span>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      <div className="text-slate-500 text-xs">Pyth Price</div>
                    ...
                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      ...
                      <div className="text-xs text-emerald-600 font-medium mt-0.5">Freshness verified</div>
                    </div>
                    ...
                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5">Prior allocation</div>
                    </div>

                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      <div className="text-slate-500 text-xs">Vault TVL</div>
                      <div className="font-semibold text-slate-900 text-sm mt-0.5">
                        ${(selectedDecision.inputs.vaultTvlUsd / 1_000_000).toFixed(2)}M
                      </div>
                      <div className="text-xs text-slate-400 mt-0.5">Anchor Vault PDA</div>
                    </div>

                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      <div className="text-slate-500 text-xs">Cash Reserves</div>
                      <div className="font-semibold text-slate-900 text-sm mt-0.5">
                        {selectedDecision.inputs.cashReservePct.toFixed(1)}%
                      </div>
                      <div className="text-xs text-emerald-600 mt-0.5">&gt; 10% Floor</div>
                    </div>

                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      <div className="text-slate-500 text-xs">Signer Isolation</div>
                      <div className="font-semibold text-emerald-600 text-sm mt-0.5">Enforced</div>
                      <div className="text-xs text-slate-400 mt-0.5">No keys in model</div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="p-4 rounded-xl bg-slate-900 text-slate-300 font-mono text-xs space-y-3">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      ...
                      <span className="text-slate-400 text-xs">Compute Units: {selectedDecision.simulation.computeUnitsUsed.toLocaleString()}</span>
                    </div>
